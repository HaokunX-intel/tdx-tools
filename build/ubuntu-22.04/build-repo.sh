...
_build_repo() {
    ...
    if [ -d ./mini-dinstall/incoming ]; then
        rm -rf ./mini-dinstall/incoming
    fi
    if [ -d ./mini-dinstall/REJECT ]; then
        rm -rf ./mini-dinstall/REJECT
    fi
    if [ -f ./mini-dinstall/mini-dinstall.log ]; then
        rm ./mini-dinstall/mini-dinstall.log
    fi
...
_build_guest_repo () {
    ...
    pushd intel-mvp-tdx-guest-shim
    cp *.build *.buildinfo *.changes *.tar.gz *.deb ../$GUEST_REPO/more/
    popd
...
_build_host_repo () {
    ...
    pushd intel-mvp-tdx-kernel
    cp *.build *.buildinfo *.changes *.tar.gz *.deb ../$HOST_REPO/more/
    popd    

    pushd intel-mvp-tdx-qemu-kvm
    cp *.build *.buildinfo *.changes *deb *.ddeb ../$HOST_REPO/more/
    popd

    pushd intel-mvp-ovmf
    cp *.build *.buildinfo *.changes *deb ../$HOST_REPO/more/
    popd    

    pushd intel-mvp-tdx-libvirt
    cp *.build *.buildinfo *.changes *.tar.xz *deb *.ddeb ../$HOST_REPO/more/
    popd
