...
pub fn retrieve_kbs_params() -> Result<KBSParams> {
    ...
    let url_name = VariableName::from_str(DEFAULT_KBS_URL_FILE)
        .expect("Failed to create variable name url");
    let mut url_bytes:Vec<u8> = vec![0u8; 4096];
    let (_url_data_size, _url_data_flags) = var_manager.read(&url_name, &mut url_bytes)
        .expect("Failed to read variable url");

    let cert_name = VariableName::from_str(DEFAULT_KBS_CERT_FILE)
        .expect("Failed to create variable name cert");
    let mut cert_bytes:Vec<u8> = vec![0u8; 4096];
    let (_cert_data_size, _cert_data_flags) = var_manager.read(&cert_name, &mut cert_bytes)
    ...
    let (_user_token_data_size, _user_token_data_flags) =var_manager.read(&user_token_name, &mut user_token_bytes)
